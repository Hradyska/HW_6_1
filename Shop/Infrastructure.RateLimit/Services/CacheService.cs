...
namespace Infrastructure.RateLimit.Services
{
    public class CacheService :  ICacheService
    {
        ...
        public async Task<T> GetAsync<T>(string key)
        {
            ...
            {
                _logger.LogInformation($"there is no value for key {key}");
            }
        ...
        public async Task SetAsync<T>(string key, T value)
                => await SerInternalAsync(key, value);          

        private async Task SerInternalAsync<T>(string key, T value,
            IDatabase redis = null!, TimeSpan? expiry = null)
        {
            ...
            await redis.SetAddAsync(cacheKey, serialized);
            _logger.LogInformation($"Cached value for key: {key}");
        }
